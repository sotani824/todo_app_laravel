<template>
    <div class="container-fruid h-100 pt-5">
        ...
        ></i
        ><span class="align-middle"> Create A New Goal</span>

        <i
            class="fa fa-plus align-middle pl-4 pr-1"
            data-toggle="modal"
            data-target="#tagModal"
        ></i
        ><span class="align-middle">Manage Tag</span>
                    ...
                    <div class="modal-body">
                        ...
                        <div v-for="(key, index) in tags" :key="index">
                            ...
                            <button
                                ...
                                v-on:click="deleteTag(tags[index].id)"
                            >
                                ✖
                            </button>
                        </div>
...
export default {
    ...
    methods: {
        ...
        addNewGoal: function() {
            ...
            axios.post("/goals", { title: this.title }).then(
                response => {
                    this.goals.length = 0;
                    for (let i = 0; i < response.data.length; i++) {
                        this.goals.push(response.data[i]);
        ...
        deleteGoal: function() {
            ...
            axios.defaults.headers["content-type"] = "application/json";
            console.log(this.id);
            axios.post(`/goals/${this.id}`, { _method: "delete" }).then(
                response => {
